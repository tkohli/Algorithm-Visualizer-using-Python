...
class window:
    ...
    def __init__(self, root, title) -> None:
        ...
        ttk.Label(self.root, text='Speed & Array Size:').grid(
            row=2, column=0)
    ...
    def display(self, N: int, a: list, rong: list):
        ...
        for i in range(N):
            self.canvas.create_rectangle(
                7+i*width+i*gap, 0, 7+(i+1)*width+i*gap, a[i], fill=rong[i])
    ...
    def start(self):
        if self.st['bubble'] is True:
            for i in range(self.N-1):
                for j in range(self.N-1-i):
                    self.display(self.N, self.data, [
                                 'purple' if a == j or a == j+1 else 'green' if a > self.N-1-i else 'dodgerblue' for a in range(self.N)])
                    time.sleep(self.speed)
                    if self.data[j] > self.data[j+1]:
                        self.display(self.N, self.data, [
                                     'red' if a == j or a == j+1 else 'green' if a > self.N-1-i else 'dodgerblue' for a in range(self.N)])
                        time.sleep(self.speed)
                        self.data[j], self.data[j +
                                                1] = self.data[j+1], self.data[j]
                        self.display(self.N, self.data, [
                                     'lime' if a == j or a == j+1 else 'green' if a > self.N-1-i else 'dodgerblue' for a in range(self.N)])
                        time.sleep(self.speed)
            ...
            for j in range(1, len(self.data)):
                ...
                i = j-1
                self.display(self.N, self.data, [
                             'purple' if a == i or a == i+1 else 'green' if a <= j else'dodgerblue' for a in range(self.N)])
                time.sleep(self.speed)
            ...
            for i in range(len(self.data)-1):
                ...
                for j in range(i+1, len(self.data)):
                    self.display(self.N, self.data, [
                                 'yellow' if a == min_index or a == i else 'green' if a <= i else 'dodgerblue' for a in range(self.N)])
                    time.sleep(self.speed)
                    if self.data[min_index] > self.data[j]:
                        self.display(self.N, self.data, [
                                     'red' if a == min_index or a == j else 'green' if a <= i else 'dodgerblue' for a in range(self.N)])
                        time.sleep(self.speed)
                        min_index = j
                if min_index != i:
                    self.data[i], self.data[min_index] = self.data[min_index], self.data[i]
                    self.display(self.N, self.data, [
                                 'lime' if a == min_index or a == i else 'green' if a <= i else 'dodgerblue' for a in range(self.N)])
                    time.sleep(self.speed)
            self.display(self.N, self.data, ['lime' for _ in range(self.N)])
            ...
            messagebox.showerror("Algorithm Visualizer",
                                 "You didn't select any sorting algorithm")
    ...
    def mergesort(self, a, front, last):
        if front < last:
            ...
            self.display(self.N, self.data, [
                         'dodgerblue' for _ in range(self.N)])
